...
with st.expander("Configuration Settings", expanded=True):
    model = st.text_input(
        "LLM Model (.gguf filename or leave blank for default from rag.py)", value=""
    )
...
if uploaded:
    ...
    for f in uploaded:
        # Use f.name for cross-platform compatibility to get filename
        out = data_dir / f.name
        # Write file contents
...
if run_btn and q.strip():
    with st.spinner("Processing request..."):
        ...
        if model.strip():
            kwargs["model"] = (
                model.strip()
            )  # Override default model if user specifies one
        ...
        if answer_text.startswith("[ERROR]"):
            # Handle LLM failures
            st.error(
                f"Answer generation failed. Check model configuration. Displaying raw retrieval results as fallback: {answer_text}"
            )
        ...
        if hits:
            ...
            st.warning(
                "No relevant sources retrieved. Please ensure documents have been ingested and the query is specific."
            )
